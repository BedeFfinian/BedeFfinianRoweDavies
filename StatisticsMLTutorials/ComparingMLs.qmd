...
But we will be comparing lots of specs of models so lets make a workflowset and then do the same but with lots of trained models. This will take a long time, so i set the grid to be just 2, really it should be set to around 10. We will also add (using option_add()) a control to the grid being used and the metric to be assessed (this will be necessary when we want to ensemble the models).
...
Lets select the best model, then see how well it performs on predicting the test data. Lets select the boost_tree model. The best_results object provides us the 'best' hyper parameter values for this model framework. This is based on a very small amount of tests here, in reality we should use many more combinations of hyper parameters to test the best ones.
