...
So far we haven't added any information about where this raster is. We can inspect it to find where is its dimensions, resolution and extent.
...
This is a big dataset and global, so lets subset it spatially. We can use our bounding box from earlier to do this but first lets convert our sf object to a spatvector object.
...
Okay lets use this climate data to look at the average temperatures (across the whole year) in our shape file of countries. Lets try two methods, take the centroid of each country then extract the average temperature for that locations or we can take the average values within each country then average.
...
We can some countries with little variation between the centroid and the overall mean. But some countries a bit difference (Ecuador for example). Obviously, we could do this is a multitude of different ways and it would depend on the objective that we had.
